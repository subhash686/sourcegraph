...
1. Navigate to the **_repo’s root folder_** (where the `package.json/tsconfig.json` is) and install depedencies with `npm install` then run `lsif-typescript index`.
